async function calculateLoanApproval() {
  ...
  const employmentStatus = parseInt(
  ...
  if (income < 0 || income > 2500000)
    validationErrors.push("Income must be between ₹0 and ₹25,00,000");
  if (creditScore < 0 || creditScore > 850)
    validationErrors.push("Credit score must be between 0 and 850");
  if (age < 18 || age > 70)
    validationErrors.push("Age must be between 18 and 70");
  if (![0, 1, 2].includes(employmentStatus))
    validationErrors.push(
      "Employment status must be 0 (Unemployed), 1 (Employed), or 2 (Self-employed)"
    );
  ...
  try {
    const response = await fetch(
      "https://loan-approval-api-xfth.onrender.com/calculate-loan",
      {
        method: "POST",
    ...
    resultElement.innerHTML = `
            ...
            <strong>Status:</strong> <span class="${
              data.is_approved ? "text-green-600" : "text-red-600"
            }">
                ${data.status}
            </span>
        `;
